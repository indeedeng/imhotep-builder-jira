...
public class TestJiraIssuesProcess {
    ...
    final JiraIssuesProcess process = new JiraIssuesProcess(date);
    ...
    public void testCompare() {
        ...
        final String[] expected1 = {"A", "In Progress", "86400", "86400", "0", "86400", "0", "0"};
        ...
        final String[] expected2 = {"B", "Closed", "86400", "86400", "0", "0", "0", "86400"};
        ...
        final String[] expected3 = {"", "", "86400", "86400", "0", "0", "0", "0"};
        Assert.assertEquals(expected3, output3.values().toArray());
    }

    @Test
    public void testGetRemainingIssues() {
        final String[] issue1 = {"A", "Pending Triage", "0", "0", "0", "0", "0", "0"};
        final String[] issue2 = {"B", "Closed", "0", "0", "0", "0", "0", "0"};
        ...
        final String[] expectedIssue = {"C", "Open", "86400", "86400", "0", "0", "0", "0"};
    ...
    public void testNewFields() {
        ...
        final String[] expected = {"A", "Open", "86400", "86400", "0", "86400"};      // If there is a new field it will set "0" as the value for that field
        Assert.assertEquals(expected, output.values().toArray());
    }

    @Test
    public void testNonApiStatuses() {
        final String[] issue = {"D", "Accepted", "0", "0", "0", "0", "0", "0"};       // "Accepted" is in the API but it isn't in the fields that were set for these tests
    ...
    public void testStatusReplacement() {
        ...
        final String[] issue = {"A", "a", "0", "0", "0", "1"};        // There currently isn't a way to check which statuses get replaced so the best it can do is "remove" the old one and set 0 as the new one
        final Map<String, String> output = process.compareAndUpdate(issue);
        final String[] expected = {"A", "a", "86400", "86400", "0", "86400"};
        Assert.assertEquals(expected, output.values().toArray());
    }
    ...
    public void testDateFilter() {
        ...
        final Map<String, String> output2 = process.compareAndUpdate(issue2);
        final String[] expected2 = {"B", "Open", "86400", "86400", "20180801"};
        Assert.assertEquals(expected2, output2.values().toArray());
